...
#   mutate(Comments = Side_cx) %>%
#   select(-Side_cx)
  # mutate(VisitType = ifelse(is.na(VisitType), "B", VisitType))
  # mutate(VisitType = as.factor(VisitType))
  # 
  # mutate(Date = as.Date(Date, format="%d/%m/%Y")) %>% # Adjust date format if needed


al0 <- readfile <- read.csv("2.RawData/ALS_Raw_TMS_extracted_n=26.csv", header=TRUE, sep=";", na.strings = c("", "NA", "NaN"), stringsAsFactors = TRUE)
# al0 <- al0 %>% #df0
#   mutate(Value_MSO = Value) %>%
#   select(-Value)
#   mutate(CondStim = ConditStim) %>%
#   select(-ConditStim)
#   mutate(Onset_Cx = Onset_cx) %>%
#   select(-Onset_cx)

# Bind the data frames together
df_combined <- bind_rows(df0, hc0)

df_combined <- bind_rows(df0, hc0, al0)
...
df1$ID <- factor(df1$ID, 
                 ...
                 labels = gsub(".*SNBR-(\\d{3}).*", "\\1", levels(df1$ID))) #.* = any extra, (\\d{3}) = 3 digits
df1$ID <- factor(df1$ID, 
                 levels = levels(df_combined$ID), 
                 labels = gsub(".*SNBR(\\d{3}).*", "\\1", levels(df1$ID)))

df1$ID <- factor(df1$ID, 
                 labels = gsub(".*AASNBR-(\\d{3}).*", "\\1", as.character(df1$ID)))

# remove these Pts, who are staff
df1 <- df1 %>%
  filter(!grepl("PSC,PSC|AASNBR-DS AASNBR-DS", ID)) %>%  # Remove rows with matching patterns
  mutate(ID = droplevels(ID))  # Drop unused factor levels

# relabel subj 55 whi is Genetic Carrier

df1 <- df1 %>%
  mutate(Group = as.character(Group), 
         Group = case_when( # mutate only when gg and control, 
          ID == "055" & Group == "Control " ~ "GC",  # Change "Control" to "GC" for ID 55
          TRUE ~ Group  # Keep other values unchanged
          )) %>%
  mutate(Group = factor(Group))  # Convert back to factor if needed
# 
# testdf <- df1 %>%
#   filter(ID != "055")  # Remove rows where ID is "055"

# # Check the result
# df1 %>% filter(ID == 055)  # View

# PROBLEMS!
# 040 is a patient and a control??
# 085 mislabelled
# GCs in with patients
# no dates in controls??

df1 <- df1 %>% 
  filter(!(ID %in% c("AASNBR040 AASNBR040   ", "SNBR040 SNBR040  ")))
...
csv_raw_filename <- sprintf("SNBR_collated_Raw_TMS_extracted_%s.csv", current_date)
xls_raw_filename <- sprintf("SNBR_collated_Raw_TMS_extracted_%s.xlsx", current_date)
...
# Plot ALS Subgroup - Bar plot by Group
...
plot <- df1 %>% 
  ...
  # mutate(VisitType = fct_relevel(VisitType, "B", "F1"),
  #        ID = as.factor(ID)) %>%  # Ensure ID is a factor for faceting
  mutate(ID = as.factor(ID), as.factor(Group))  %>%
  select(ID, Group, Value_MSO, ISI_ms) %>%  
  group_by(ISI_ms, ID, Group) %>%  
  summarise(Ave_RMT_Change = mean(Value_MSO, na.rm = TRUE),
            Ave_SD = sd(Value_MSO, na.rm = TRUE),
            .groups = "drop") %>%  # Avoid unnecessary grouping in ggplot
  ggplot(aes(x = Group, y = Ave_RMT_Change, 
             ymin = Ave_RMT_Change - Ave_SD, 
             ymax = Ave_RMT_Change + Ave_SD, 
             group = Group, color = as.factor(ISI_ms))) +  # Use `group` and ensure color is a factor
  # geom_hline(yintercept = 108, linetype = "dashed", color = "black") +  # Add dashed line at y = 100
  # geom_hline(yintercept = 112, linetype = "dashed", color = "blue") +  # Add dashed line at y = 108 - 98 for HC data
  # geom_hline(yintercept = 95, linetype = "dashed", color = "red") +  # Add dashed line at y = 108 - 98 for HC data
  # geom_hline(yintercept = 98, linetype = "dashed", color = "red") +  # Add dashed line at y = 108 - 98 for HC data
  ...
  geom_line(position = position_dodge(width = 0.3)) +  # Connect points per ISI_ms
  scale_y_continuous(breaks = seq(80, 140, by = 10)) +
  # facet_wrap(~Group) +  # Correct use of faceting
  theme_grey() +
  theme(axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
        strip.text = element_text(size = 14),
...
# Create the summarized data for scatter plot
summarized_data <- df1 %>% 
  filter(Test %in% c('tSICIp')) %>% 
  filter(Muscle %in% c('FDI')) %>% 
  # filter(ISI_ms %in% c(1.5, 2)) %>% 
  # filter(VisitType == 'B') %>%  
  mutate(ID = as.factor(ID),
         Group = fct_relevel(Group, "Control", "patient", "TOF", "GC")) %>%  # Reorder Group
...
plot <- ggplot() + 
  ...
  geom_boxplot(data = df1 %>% 
                 ...
                 # filter(ISI_ms %in% c(1.5, 2)) %>% 
                 # filter(VisitType == 'B') %>%  
                 mutate(Group = fct_relevel(Group, "Control", "patient", "TOF", "GC")), 
               aes(x = VisitType, y = Value_MSO, fill = as.factor(ISI_ms)), 
               alpha = 0.3, position = position_dodge(width = 0.3)) +  # Boxplot layer
  
  # Scatter plot with error bars using summarized data
  geom_pointrange(data = summarized_data, 
                  aes(x = VisitType, y = Ave_RMT_Change, 
                      ymin = Ave_RMT_Change - Ave_SD, 
                      ymax = Ave_RMT_Change + Ave_SD, 
                      group = ISI_ms, color = as.factor(ISI_ms)),
                  position = position_dodge(width = 0.3)) +  # Points with error bars
  ...
  ggtitle("SNBR SICI Outcomes by Group") +
  xlab("Time") + ylab("SICI (%MSO)") +
  
  # Line to connect points per ISI_ms
  geom_line(data = summarized_data, 
